...
7. **YouTube Data API key + OAuth client (Google Cloud project)** — added 2026-07-16 with the media-kit spec; expanded same day after the founder made OAuth demographics a pre-launch requirement. Needed: (a) API key with YouTube Data API v3 enabled (public channel stats, ~1–3 quota units/fetch of a 10K/day default); (b) OAuth client ID/secret + consent screen with the YouTube Analytics readonly scope (demographics pull). Google's consent-screen verification can take days–weeks — start early. Env var names TBD with the `/core/youtube` implementation.
